...
typedef struct type_info_base_list {	//	type info base list
	...
	int	 				offset;			//	offset of base in main class (0x80000000 : ambiguous/no access list follows
}	type_info_base_list;
...
typedef struct type_info_ambighead {	//	type ambiguous/no access base list header
	...
	int					offset;			//	offset of base in main class (|=0x80000000)
	int					bases;			//	number of type_info_base_list elements (public bases)
}	type_info_ambighead;
...
typedef struct RTTIVTableHeader {		//	RTTI header in a vtable
	...
	int					complete_offset;//	offset of complete class
}	RTTIVTableHeader;
...
//
	if(obj==0) throw _STD::bad_typeid();  //cc 991118
...
void *__dynamic_cast(void *obj,long offset,type_info_struct *typeinfo,type_info_struct *subtypeinfo,short isref)
{
	...
	if(isref) throw _STD::bad_cast();  // cc991118
