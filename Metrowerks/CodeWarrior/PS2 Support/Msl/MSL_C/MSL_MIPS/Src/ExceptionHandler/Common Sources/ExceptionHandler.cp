...
typedef void (*DeleteFunc)(address);	/*	type of 'operator delete' function	*/
...
typedef struct ex_catchblock {
	...
	unsigned int	pcoffset;			/*	function-relative offset to catch label	*/
	int 			cinfo_ref;			/*	frame-relative offset to local CatchInfo struct	*/
} ex_catchblock;
...
typedef struct ex_specification {
	unsigned int	nspecs;				/*	number of specifications (0-n)	*/
	unsigned int	pcoffset;			/*	function-relative offset to catch label	*/
	int 			cinfo_ref;			/*	frame-relative offset to local CatchInfo struct	*/
 ...
 *
 */
static ExceptionTableIndex *BinarySearch(ExceptionTableIndex *table, 
...
static void FindExceptionRecord(char *return_address, ExceptionInfo *info)
{
	...
	exceptiontable = BinarySearch(info->exception_table_start,
	...
	if (DIRECT_STORED(exceptiontable)) {
		...
		info->exception_record = __Relocate(exceptiontable->exception_table);
	}		
 ...
 *		char*			catch_type;	//	absolute/relative pointer to catch type info
...
inline void DecodeCatchBlock(char *ap, ex_catchblock *cb)
{
	...
	cb->catch_type = (cb->catch_type ? (__Relocate(cb->catch_type)) : 0);
...
inline void Branch(ExceptionInfo *info, ThrowContext *) /* cc removed context */
{
	int target;
...
inline void DestroyLocalArray(int apply, ExceptionInfo *info, ThrowContext *context)
{
	char *ap = info->action_pointer;
	int array;
	unsigned int nelements, elemsize, i;
	void *dtor;
	char *arrayptr;
...
inline void DeletePointer(int apply, ExceptionInfo *info, ThrowContext *context)
{
	...
	if (apply) {
		if (regptr)
			objectptr = (address) __Register(context, ptr);
		else
			objectptr = *(address *)__LocalVariable(context, ptr);
...
inline void CatchBlock(int apply, ExceptionInfo *info, ThrowContext *) /* cc removed context */
{
	char *ap = info->action_pointer;
	char *catch_type;
	...
	ap += sizeof(exaction_type);
	ap = __DecodeAddress(ap, &catch_type);	catch_type = __Relocate(catch_type);
...
inline void Specification(int apply, ExceptionInfo *info, ThrowContext *) /* cc removed context unused arg */
{
	char *ap = info->action_pointer;
	unsigned int nspecs, pcoffset;
	int cinfo_ref;
...
static CatchInfo *FindMostRecentException(ThrowContext *context, ExceptionInfo *info)
{
	...
	context->catchinfo = catchinfo;
		...
		/*	return the pointer to the catch info struct for the most recent exception	*/
...
static int IsInSpecification(char *extype, ex_specification *spec)
{
	...
	for (i = 0; i < spec->nspecs; i++) {
		__DecodeAddress(specptr, &spectype);	spectype = __Relocate(spectype);
...
static void SetupCatchInfo(ThrowContext *context, long cinfo_ref, long result_offset)
{
	...
	if (*context->throwtype == '*') {
		...
		catchinfo->pointercopy	= *(int *)context->location + result_offset;
	}
	else {
